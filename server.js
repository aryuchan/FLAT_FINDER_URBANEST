...
const auth = (roles = []) => async (req, res, next) => {
  ...
  try {
    ...
    req.user = decoded;
    if (roles.length && !roles.includes(decoded.role)) return res.status(403).json({ success: false, message: 'Forbidden' });
    next();
  } catch (err) {
...
app.post('/api/signup', authLimiter, async (req, res) => {
  try {
    ...
    await query('INSERT INTO users (id, name, email, password, role) VALUES (?,?,?,?,?)', [id, name, email.toLowerCase().trim(), hashed, role]);
    res.json({ success: true, message: 'Account created' });
  } catch (err) {
    res.status(400).json({ success: false, message: err.errors?.[0]?.message || 'Invalid input' });
  }
});
...
app.post('/api/login', authLimiter, async (req, res) => {
  try {
    ...
    if (!user || !(await bcrypt.compare(password, user.password)) || user.status === 'suspended') {
      return res.status(401).json({ success: false, message: 'Invalid credentials' });
    }
    const token = jwt.sign({ id: user.id, name: user.name, role: user.role }, JWT_KEY, { expiresIn: '7d' });
    res.cookie('ff_token', token, { httpOnly: true, secure: IS_PROD, sameSite: 'strict', maxAge: 7 * 86400000 });
...
app.post('/api/flats/:id/image', auth(['owner']), upload.single('image'), async (req, res) => {
  try {
...
app.patch('/api/flats/:id', auth(['owner', 'admin']), async (req, res) => {
  ...
  await query('UPDATE flats SET available = ? WHERE id = ?', [available ? 1 : 0, req.params.id]);
  res.json({ success: true });
});
...
app.post('/api/bookings', auth(['tenant']), async (req, res) => {
  ...
  const id = crypto.randomUUID();
  await query('INSERT INTO bookings (id, flat_id, tenant_id, check_in, check_out) VALUES (?,?,?,?,?)', [id, flat_id, req.user.id, check_in, check_out]);
  res.json({ success: true });
});
