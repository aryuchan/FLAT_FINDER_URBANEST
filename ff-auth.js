...
const Auth = {
  ...
  async renderLogin() {
    if (appState.currentUser)
      return this.redirectToPortal(appState.currentUser.role);
    await render(
      populateTemplate("tmpl-auth", {
        title: "Sign In",
  ...
  async signup(data, btn) {
    if (!data.name || data.name.trim().length < 2)
      return showToast("Name too short", "warning");
    if (!data.password || data.password.length < 8)
      return showToast("Password min 8 chars", "warning");
